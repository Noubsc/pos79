                            ...
                            <tbody>
                              ...
                              <tr v-for="(detail, index) in details" :key="index">
                                <td>
                                  ...
                                  <span class="badge badge-success" style="font-size: 15px;">{{detail.name}}</span>
                                  <br>
                                  <span>{{detail.code}}</span>
                                  <i @click="Modal_Update_Detail(detail)" class="i-Edit"></i>
                                </td>
                                <td>{{currentUser.currency}} {{formatNumber(detail.Total_price)}}</td>
                                <td>
                                  <div class="quantity">
                                    <b-input-group>
                                 ...
                                 <td class="text-center">{{currentUser.currency}} {{formatNumber(detail.subtotal)}}</td>
                                <td>
                                  <a
                    ...
                    <b-row>
                      <b-col md="12">
                        <div class="grandtotal">
                          <span>{{$t("Total")}} : {{currentUser.currency}} {{formatNumber(GrandTotal)}}</span>
                        </div>
                      </b-col>
                ...
                >
                  ...
                  <div class="flex-grow-1 d-bock">
                    ...
                    >
                      ...
                      >{{currentUser.currency}} {{formatNumber(product.Net_price)}}</span>
                      <p
                        class="m-0 text-muted text-small w-15 w-sm-100 d-none d-lg-block item-badges"
                      >
                        <span
                          class="badge badge-info"
                        >{{formatNumber(product.qte_sale)}} {{product.unitSale}}</span>
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div id="invoice-POS">
            <div style="max-width:400px;margin:0px auto">
              <div style="text-align: center;">
                <img :src="'/images/'+currentUser.logo"  alt width="60" height="60"></img>
              </div>
                ...
                <tbody>
                  <tr v-for="detail_invoice in invoice_pos.details">
                    <td colspan="3">
                      <span>
                        {{detail_invoice.name}}
                        <br>
                        {{formatNumber(detail_invoice.quantity)}} {{detail_invoice.unit_sale}} x {{formatNumber(detail_invoice.total/detail_invoice.quantity)}}
                      </span>
                    </td>
                    <td
                      style="text-align:right;vertical-align:bottom"
                    >{{formatNumber(detail_invoice.total)}}</td>
                  </tr>

                  <tr style="margin-top:10px">
                    <td colspan="3" class="total">{{$t('OrderTax')}}</td>
                    <td style="text-align:right;" class="total">{{invoice_pos.symbol}} {{formatNumber1(invoice_pos.sale.taxe)}} ({{formatNumber1(invoice_pos.sale.tax_rate,2)}} %)</td>
                  </tr>

                  <tr style="margin-top:10px">
                    <td colspan="3" class="total">{{$t('Discount')}}</td>
                    <td style="text-align:right;" class="total">{{invoice_pos.symbol}} {{formatNumber(invoice_pos.sale.discount)}}</td>
                  </tr>
                  ...
                  <tr style="margin-top:10px">
                    ...
                    <td
                      ...
                      class="total"
                    >{{invoice_pos.symbol}} {{formatNumber(invoice_pos.sale.GrandTotal)}}</td>
                  </tr>
                  ...
                  <tr v-show="invoice_pos.sale.paid_amount < invoice_pos.sale.GrandTotal">
                    ...
                    <td
                      style="text-align:right;"
                      class="total"
                    >{{invoice_pos.symbol}} {{formatNumber(invoice_pos.sale.paid_amount)}}</td>
                  </tr>
                  ...
                  <tr v-show="invoice_pos.sale.paid_amount < invoice_pos.sale.GrandTotal">
                    ...
                    <td
                      style="text-align:right;"
                      class="total"
                    >{{invoice_pos.symbol}} {{parseFloat(invoice_pos.sale.GrandTotal - invoice_pos.sale.paid_amount)}}</td>
                  </tr>
                </tbody>
              </table>
                ...
                <tbody>
                  <tr v-for="payment_pos in payments">
                    ...
                    <td
                      ...
                      colspan="2"
                    >{{formatNumber(payment_pos.montant)}}</td>
                    <td
                      style="text-align: right;"
                      colspan="1"
                    >{{formatNumber(payment_pos.change)}}</td>
                  </tr>
                </tbody>
              </table>
                ...
                <b-col md="6">
                  <b-row>
                    ...
                    <b-col lg="12" md="12" sm="12">
                      ...
                      <p
                        class="change_amount"
                      >{{parseFloat(payment.received_amount - payment.amount)}}</p>
                    </b-col>
...
export default {
  ...
  methods: {
    ...
    print_pos() {
      ...
      a.document.write(
        '<style>@media print { img { display: none; }, img { display: block; }}</style>'
      );
      
      a.document.write("<body >");
      a.document.write(divContents);
      a.document.write("</້></html>");
      a.document.close();
      a.print();
    },
      ...
      if (error) {
        ...
        axios
          .post("pos/CreatePOS", {
            ...
            received_amount : parseFloat(this.payment.received_amount),
            change: parseFloat(this.payment.received_amount - this.payment.amount),
            token: token.id
          })
          .then(response => {
            this.paymentProcessing = false;
  ...
  created() {
    ...
    Fire.$on("pay_now", () => {
      setTimeout(() => {
        ...
        this.payment.received_amount = this.GrandTotal;

        this.payment.Reglement = "ເງິນສົດ";
