          ...
          <b-col md="8">
            <table class="table table-hover table-bordered table-md">
              <tbody>
                ...
                <tr>
                  <td>{{$t('Cost')}}</td>
                  <th>{{currentUser.currency}} {{formatNumber(product.cost, 0)}}</th>
                </tr>
                <tr>
                  <td>{{$t('Price')}}</td>
                  <th>{{currentUser.currency}} {{formatNumber(product.price, 0)}}</th>
                </tr>
                <tr>
                  <td>{{$t('Unit')}}</td>
                  <th>{{product.unit}}</th>
                ...
                <tr>
                  <td>{{$t('Tax')}}</td>
                  <th>{{formatNumber(product.taxe ,0)}} %</th>
                </tr>
                ...
                <tr>
                  ...
                  <th>
                    ...
                    >{{formatNumber(product.stock_alert ,0)}}</span>
                  </th>
                </tr>
            ...
            <table class="table table-hover table-sm">
              ...
              <tbody>
                <tr v-for="PROD_W in product.CountQTY">
                  <td>{{PROD_W.mag}}</td>
                  <td>{{formatNumber(PROD_W.qte ,0)}} {{product.unit}}</td>
                </tr>
              </tbody>
            </table>
          </b-col>
            ...
            <table class="table table-hover table-sm">
              ...
              <tbody>
                <tr v-for="PROD_V in product.CountQTY_variants">
                  ...
                  <td>{{formatNumber(PROD_V.qte ,0)}} {{product.unit}}</td>
                </tr>
              </tbody>
            </table>
          </b-col>
...
export default {
  ...
  methods: {
    ...
    formatNumber(number, dec) {
      ...
      //   : number.toString()
      // ).split(".");
      // if (dec <= 0) return value[0].toLocaleString();
      
      // let formated = value[1] || "";
      // if (formated.length > dec)
      //   return `${value[0]}.${formated.substr(0, dec)}`;
      // while (formated.length < dec) formated += "0";
      // return `${value[0].toLocaleString()}.${formated}`;
