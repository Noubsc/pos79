...
class ReportController extends BaseController
{
    ...
    public function Report_Sales(request $request)
    {
        ...
        // /api/report/Sales?page=1&Ref=&date=2023-10-01&client_id=&statut=&payment_statut=&SortField=id&SortType=desc&search=&limit=10
        //->where('date', '=', $ddd)
        ...
        foreach ($Sales as $Sale) {
            ...
            $item['due'] = $Sale['GrandTotal'] - $Sale['paid_amount'];
            $item['payment_status'] = $Sale['payment_statut'];
            $item['notes'] = $Sale['facture'][0]->Reglement ?? "Null";
            // $item['notes'] = $Sale['facture'];
            // $a = $Sale['facture'];
            // foreach($a as $prop) {
            //     $first_prop = $prop;
            //     break; // exits the foreach loop
            //     } 
            // $item['notes'] = $first_prop->Reglement;
    ...
    public function ProfitAndLoss(request $request)
    {
        ...
        $item['cash'] = PaymentSale::whereBetween('date', array($request->from, $request->to))
            ->where('Reglement','=','ເງິນສົດ')
            ->select(
                DB::raw('SUM(montant) AS sum')
            )->first();
        $item['tran'] = PaymentSale::whereBetween('date', array($request->from, $request->to))
            ->where('Reglement','=','ເງິນໂອນ')
            ->select(
                DB::raw('SUM(montant) AS sum')
            )->first();

        $item['dasdue'] = Sale::where('deleted_at', '=', null)->where('payment_statut','=','unpaid')->whereBetween('date', array($request->from, $request->to))
            ->select(
                DB::raw('SUM(GrandTotal) AS sum')
            )->first();
